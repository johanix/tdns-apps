...
#   e.g.   mkpkg.sh tdns-zonegen /usr/local bin/tdns-zonegen
#
# This is a near-verbatim sibling of tdns's utils/mkpkg.sh, which is itself a
# sibling of labstuff's. The duplication is deliberate and now three-way: they
# are separate repos, and vendoring one into the others is worse than keeping
# copies that rarely change. If you fix something here, fix it in
# tdns/utils/mkpkg.sh and labstuff/utils/mkpkg.sh too.
#
...
#
# For tdns-apps the driver is different but lands in the same place: these are
# operator tools that run on the lab master, and "which build of tdns-zonegen
# generated these zone files?" is a question worth being able to answer from
...
# One package per app, so a single hardcoded one-line COMMENT would describe
# none of them. Callers pass PKGCOMMENT; the default below is the fallback for
# a hand-run invocation.
COMMENT="${PKGCOMMENT:-$PKGBASE, part of the tdns-apps toolset}"
...
installed into a VM image before first boot.
EOF
